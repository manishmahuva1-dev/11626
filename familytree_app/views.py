...
def family_member_list(request):
    if request.user.is_superuser:
        # If the user is a superuser, show all family members
        family_members = FamilyMember.objects.all().order_by('-id')
    else:
        # For non-superusers, exclude family members with city_id=72
        family_members = FamilyMember.objects.exclude(city_id=72).order_by('-id')
...
def family_member_detail(request, pk):
    ...
    # Check if the family member has city_id=72
    if family_member.city_id == 72 and not request.user.is_superuser:
        return HttpResponseForbidden("Access Denied: Family member with city_id=72 cannot be viewed.")
...
def search_by_name(request):
    ...
    if request.user.is_superuser:
        ...
        # For non-superusers, exclude family members with city_id=72
        family_members = FamilyMember.objects.exclude(city_id=72).order_by('-id')
        #family_members = FamilyMember.objects.exclude(gender="F").order_by('-id')
    ...
    selected_surname = request.GET.get('selected_surname')

    if selected_surname:
        family_members = family_members.filter(surname=selected_surname)
    ...
    return render(request, 'familytree_app/family_member_list.html', {
        ...
        # 'all_surnames': all_surnames,  # Add this line
...
from .forms import CommentsForm  # Create a form for comments if not already done
...
def feedback(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            # Optionally, you can redirect the user to a thank you page or the home page
            return redirect('familytree_app:home')
    else:
        form = CommentsForm()

    return render(request, 'familytree_app/feedback.html', {'form': form})
